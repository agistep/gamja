...
class JDBCEventStorage extends OptimisticLockingSupport {
    ...
    public void lockedSave(Event anEvent) {
        ...
        if (Objects.isNull(serializer)) {
            serializer = supportedSerializer().stream()
        ...
        try (PreparedStatement prep = conn.prepareStatement(INSERT_DML)){
            prep.setLong(1, id);
            prep.setLong(2, seq);
    ...
    public List<Event> findByAggregate(long id) {
        ...
        try(PreparedStatement prep = conn.prepareStatement(SELECT_QUERY)) {
            prep.setLong(1, id);
